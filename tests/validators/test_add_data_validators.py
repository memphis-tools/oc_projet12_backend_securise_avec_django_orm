...
def test_add_client_data_is_not_valid_without_all_expeted_data(dummy_client_data):
    dummy_client_data.pop("company_id")
    ret = add_data_validators.add_client_data_is_valid(dummy_client_data)
    assert ret is False


def test_add_collaborator_data_is_not_valid_without_all_expeted_data(
...
def test_add_collaborator_department_data_is_not_valid_without_all_expeted_data(
    dummy_collaborator_department_data,
):
...
def test_add_collaborator_role_data_is_not_valid_without_all_expeted_data(
    dummy_collaborator_role_data,
):
...
def test_add_event_data_is_not_valid_without_all_expeted_data(dummy_event_data):
    dummy_event_data.pop("event_id")
    ret = add_data_validators.add_event_data_is_valid(dummy_event_data)
    assert ret is False


def test_add_location_data_is_not_valid_without_all_expeted_data(dummy_location_data):
