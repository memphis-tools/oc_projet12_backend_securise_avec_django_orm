...
def test_collaborator_manipulation(
    ...
    result = ConsoleClientForCreate(db_name).add_collaborator(
        collaborator_attributes_dict=dummy_collaborator_data_2
    )
    print(result)
    assert isinstance(result, str)

    result = ConsoleClientForUpdate(db_name=db_name).update_collaborator(
        custom_partial_dict=dummy_collaborator_partial_data
    )
    print(result)
    assert isinstance(result, str)

    matricule = dummy_collaborator_data_2["registration_number"]
    result = ConsoleClientForDelete(db_name=db_name).delete_collaborator(
        collaborator_custom_id=matricule
    )
    print(result)
    assert isinstance(result, str)
...
def test_contract_manipulation(
    ...
    result = ConsoleClientForUpdate(db_name=db_name).update_contract(
        custom_partial_dict=dummy_contract_partial_data
    )
    assert isinstance(result, str)
    assert "Update" in result

    result = ConsoleClientForDelete(db_name=db_name).delete_contract(
